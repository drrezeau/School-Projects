...
int findInverse(int a, int m)//brute forcing it
{
   int i = 1;
   while ((a * i) % m != 1)
      i++;
...
/*
  elegant way and very cohesive way of finding the inverse
int findInverse(int a, int m)
{
   int s;
   int t;
   int gcd = extendedEuclideanGCD(a, m, s, t); //need to define in program
   assert(gcd == 1);
   if (s < 0)
   {
   s += m;
